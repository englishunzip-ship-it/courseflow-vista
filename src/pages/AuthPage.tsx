...
import { Copy, Check, Eye, EyeOff, ExternalLink, Image, Link2, Upload } from "lucide-react";
...
export default function AuthPage() {
  ...
  const [screenshotMode, setScreenshotMode] = useState<ScreenshotMode>("file");
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (screenshotMode === "file" && screenshotFile) {
        finalScreenshotUrl = await uploadToImgBB(screenshotFile);
      } else if (screenshotMode === "url" && screenshotUrl) {
    ...
    <div className="p-4 max-w-md mx-auto mt-4 animate-fade-in">
      ...
      {isLogin ? (
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          {/* Screenshot Upload - File or URL */}
          <div className="space-y-2">
            <p className="text-sm font-medium text-foreground">Payment Screenshot</p>
            ...
            <div className="flex bg-card rounded-lg border border-border overflow-hidden">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 py-2 text-xs font-medium transition-colors ${
                  screenshotMode === "file" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"
                }`}
              >
              ...
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 py-2 text-xs font-medium transition-colors ${
                  screenshotMode === "url" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"
                }`}
              >
